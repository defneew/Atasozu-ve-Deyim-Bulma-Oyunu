...
class RegisterPage(QMainWindow):
    ...
    def Register(self):
        ...
        check_query = "SELECT COUNT(*) FROM tbl_person WHERE personMail = ?"
        self.operation.execute(check_query, (personMail,))
        if self.operation.fetchone()[0] > 0:
            self.registerform.statusbar.showMessage("Bu email adresi zaten kayıtlı...", 10000)
            self.connect.close()
            return        
        try: 
