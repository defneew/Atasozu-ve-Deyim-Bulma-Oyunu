...
class CategoryPage(QMainWindow):
    ...
    def Category(self):
        ...
        if category == "Kelime":
            ...
            return
        if length:
            take = "SELECT tbl_adw.text, tbl_adw.adwID, tbl_adw.categoryID, tbl_adw.length FROM tbl_adw INNER JOIN tbl_category ON tbl_adw.categoryID = tbl_category.categoryID WHERE tbl_category.categoryName = ? AND tbl_adw.length = ? AND tbl_adw.adwID NOT IN (SELECT adwID FROM tbl_texts WHERE personID = ?)" 
